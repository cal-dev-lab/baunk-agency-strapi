(globalThis["webpackChunkstrapicms"] = globalThis["webpackChunkstrapicms"] || []).push([[2427],{
...
const customPermissions=(()=>{if(window&&window.strapi&&window.strapi.isEE){return (__webpack_require__(601)/* ["default"] */ .Z);}return (__webpack_require__(57050)/* ["default"] */ .Z);})();const permissions_permissions=(0,lodash.merge)(defaultPermissions,customPermissions);/* harmony default export */ const src_permissions = (permissions_permissions);
...
const AuthPage_forms=(()=>{if(window&&window.strapi&&window.strapi.isEE){return (__webpack_require__(66267)/* ["default"] */ .Z);}return (__webpack_require__(48858)/* ["default"] */ .Z);})();const AuthPage=_ref=>{let{hasAdmin,setHasAdmin}=_ref;const{push,location:{search}}=(0,react_router_dom_min.useHistory)();const{changeLocale}=(0,useLocalesProvider/* default */.Z)();const{setSkipped}=(0,build.useGuidedTour)();const{trackUsage}=(0,build.useTracking)();const{params:{authType}}=(0,react_router_dom_min.useRouteMatch)('/auth/:authType');const query=(0,build.useQuery)();const{Component,endPoint,fieldsToDisable,fieldsToOmit,inputsPrefix,schema,...rest}=get_default()(AuthPage_forms,authType,{});const[{formErrors,modifiedData,requestError},dispatch]=(0,react.useReducer)(reducer,initialState,AuthPage_init);const CancelToken=(axios_default()).CancelToken;const source=CancelToken.source();(0,react.useEffect)(()=>{// Cancel request on unmount
return()=>{source.cancel('Component unmounted');};// eslint-disable-next-line react-hooks/exhaustive-deps
},[]);// Reset the state on navigation change
(0,react.useEffect)(()=>{dispatch({type:'RESET_PROPS'});},[authType]);const handleChange=_ref2=>{let{target:{name,value}}=_ref2;dispatch({type:'ON_CHANGE',keys:name,value});};const handleSubmit=async(e,_ref3)=>{let{setSubmitting,setErrors}=_ref3;setSubmitting(true);const body=omit_default()(e,fieldsToOmit);const requestURL=`/admin/${endPoint}`;if(authType==='login'){await loginRequest(e,requestURL,{setSubmitting,setErrors});}if(authType==='register'||authType==='register-admin'){await registerRequest(e,requestURL,{setSubmitting,setErrors});}if(authType==='forgot-password'){await forgotPasswordRequest(body,requestURL,{setSubmitting,setErrors});}if(authType==='reset-password'){await resetPasswordRequest(body,requestURL,{setSubmitting,setErrors});}};const forgotPasswordRequest=async(body,requestURL,_ref4)=>{let{setSubmitting,setErrors}=_ref4;try{await axios_default()({method:'POST',url:`${strapi.backendURL}${requestURL}`,data:body,cancelToken:source.token});push('/auth/forgot-password-success');}catch(err){console.error(err);setErrors({errorMessage:'notification.error'});}finally{setSubmitting(false);}};const loginRequest=async(body,requestURL,_ref5)=>{let{setSubmitting,setErrors}=_ref5;try{const{data:{data:{token,user}}}=await axios_default()({method:'POST',url:`${strapi.backendURL}${requestURL}`,data:omit_default()(body,fieldsToOmit),cancelToken:source.token});if(user.preferedLanguage){changeLocale(user.preferedLanguage);}build.auth.setToken(token,body.rememberMe);build.auth.setUserInfo(user,body.rememberMe);redirectToPreviousLocation();}catch(err){if(err.response){const errorMessage=get_default()(err,['response','data','error','message'],'Something went wrong');if(camelCase_default()(errorMessage).toLowerCase()==='usernotactive'){push('/auth/oops');dispatch({type:'RESET_PROPS'});return;}setErrors({errorMessage});}}finally{setSubmitting(false);}};const registerRequest=async(body,requestURL,_ref6)=>{let{setSubmitting,setErrors}=_ref6;try{trackUsage('willCreateFirstAdmin');const{data:{data:{token,user}}}=await axios_default()({method:'POST',url:`${strapi.backendURL}${requestURL}`,data:omit_default()(body,fieldsToOmit),cancelToken:source.token});build.auth.setToken(token,false);build.auth.setUserInfo(user,false);setSubmitting(false);setHasAdmin(true);const{roles}=user;if(roles){const isUserSuperAdmin=roles.find(_ref7=>{let{code}=_ref7;return code==='strapi-super-admin';});if(isUserSuperAdmin){utils_persistStateToLocaleStorage.setSkipped(false);setSkipped(false);trackUsage('didLaunchGuidedtour');}}if(authType==='register'&&body.userInfo.news===true||authType==='register-admin'&&body.news===true){push({pathname:'/usecase',search:`?hasAdmin=${hasAdmin}`});return;}redirectToPreviousLocation();}catch(err){trackUsage('didNotCreateFirstAdmin');if(err.response){const{data}=err.response;const apiErrors=(0,formatAPIErrors/* default */.Z)(data);setErrors({apiErrors});}}};const resetPasswordRequest=async(body,requestURL,_ref8)=>{let{setErrors,setSubmitting}=_ref8;try{const{data:{data:{token,user}}}=await axios_default()({method:'POST',url:`${strapi.backendURL}${requestURL}`,data:{...body,resetPasswordToken:query.get('code')},cancelToken:source.token});build.auth.setToken(token,false);build.auth.setUserInfo(user,false);// Redirect to the homePage
push('/');}catch(err){if(err.response){const errorMessage=get_default()(err,['response','data','message'],'Something went wrong');const errorStatus=get_default()(err,['response','data','statusCode'],400);dispatch({type:'SET_REQUEST_ERROR',errorMessage,errorStatus});setErrors({errorMessage});}}finally{setSubmitting(false);}};const redirectToPreviousLocation=()=>{if(authType==='login'){const redirectTo=query.get('redirectTo');const redirectUrl=redirectTo?decodeURIComponent(redirectTo):'/';push(redirectUrl);}else{push('/');}};// Redirect the user to the login page if
...
const customRoutes=(()=>{if(window&&window.strapi&&window.strapi.isEE){return (__webpack_require__(33948)/* ["default"] */ .Z);}return (__webpack_require__(36248)/* ["default"] */ .Z);})();/* harmony default export */ const routes = ([...customRoutes,...utils_defaultRoutes]);
...
const customGlobalLinks=(()=>{if(window&&window.strapi&&window.strapi.isEE){return (__webpack_require__(43202)/* ["default"] */ .Z);}return (__webpack_require__(41729)/* ["default"] */ .Z);})();/* harmony default export */ const globalLinks = ([...utils_defaultGlobalLinks,...customGlobalLinks]);
